...
### 1. [Object Poolパターンによるメモリ管理とGC対策](【ここにPlayerShooter.csのURLを貼る】)
...
### 2. [列挙型(enum)を用いた、拡張性の高い状態管理](【ここにItem.csのURLを貼る】)
動画で取得している「バリア」などのアイテム種類や、敵のステータス管理において、マジックナンバー（0や1などの直接的な数値）を排除し、`enum` を採用しました。
